...
**현재 상태의 단일 소스는 [circular-design.md](circular-design.md) "현재 위치"** (여기에 복제하지 않는다 — 두 곳이 따로 갱신되며 어긋난 전례). 요약(2026-09-23): M1·M3·M5 ✅, M2 뼈대 ✅, 전투 구조 ✅(효과×경로, `HitShape`, 무기 10종, 적 공격 W7), 적 4종·피격·사망 ✅, 1920×1080 고정 시야 + 무한 필드 ✅, 테스트 씬 콜라이더 표시 ✅ → **다음 M4(사용자 패턴 상세 대기)**.
...
| **M3** | 적 종류: 근접·탱커·원거리·마법 (`mobs.csv` + `mob_attacks.csv`) + **몹 접촉 피해·플레이어 사망** + 적 공격(전투 구조 W7) — B-적 | ✅ (헤드리스 검증, 4종 GRUNT/BRUTE/ARCHER/WITCH, 피격 무적 0.5초, HP 0 → RETRY/LOBBY — [circular-design.md](circular-design.md) §5, 보스는 M6) |
| M4 | 스폰 패턴 3종(일방통행·사각 포위·좌우 줄) + 시간표 CSV, 패턴 예고, 임시 돌진 삭제 — B-스폰 | ❌ **다음** (**패턴 상세는 사용자가 추가 예정 — 착수 전 확인**) |
| **M5** | **무기 5종**(검=부채꼴, 채찍=직선, 스태프=폭발형 전기볼트, 단검=관통2 투사체, 트럼프 카드=랜덤 데미지 투사체) + 캐릭터별 시작 무기 배정 + `weapons.csv` + 장신구(`accessories.csv`) + 무기/장신구 슬롯 분리(6/6, 최대 Lv5) + **오버플로우**(궁극기 내용 [미정], Q 게이지만) — B-무기장신구, [기초]+[확정] | ✅ 전체 완료(헤드리스 검증) — 남은 건 단검 밸런스 재측정뿐. 구현 기록은 [circular-design.md](circular-design.md) §3 |
...
**전투 구조 ✅(2026-09-23)**: 판정 도형·경로(궤도·소용돌이·럴커)·연출·CSV 를 한 구조로 — W1~W16 구현(W7 적 공격은 M3 에서 완료), 남은 W8(스프라이트)은 M7 때. [circular-combat.md](circular-combat.md) §3. W14(`CircularCombat` 분리)는 `Simulation` god class 분해의 시작이기도 하다 — 나머지(스폰·레벨업·3D 디펜스) 분리는 후보로 남긴다.
