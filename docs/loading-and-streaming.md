...
> 참고: 이 문서가 쓰는 `GameState::Title` 은 현재 코드에서 `GameState::Menu`(로비 화면)로 바뀌었고 타이틀 화면 자체가 삭제됐다 — 아래 "Title" 은 그 메뉴로 읽는다([scene-flow-design.md](scene-flow-design.md)).
> 서큘러의 **로딩 씬·타이틀 씬은 기초 설계가 요구하는 항목**이고(M8 — [circular-design.md](circular-design.md) §7.6), 그때 이 문서의 커튼/비동기 로딩이 실제 소비처를 얻는다.
...
10. **실패 격리** — 나쁜 에셋은 `Entry` 를 `Failed` 로 표시하고 커튼에 보고(재시도 / 로비로). 크래시 없음, 반쯤 로드된 월드 설치 없음.
