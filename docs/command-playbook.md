...
| 3c | "FBX 모델 그려" | **됨(정적)**: `ModelMeshPass3D` 가 시작 시 FBX 로드 → 바인드 포즈로 렌더. `main.cpp` 에서 `AddRenderPass`. 남은 일은 3d·3e | | `src/render/r3d/ModelMeshPass3D.*`, `src/main.cpp` |
| 3d | "모델에 텍스처 입혀" | TGA/이미지 로더(작은 TGA 리더 또는 stb_image vendor) + `ModelMeshPass3D` 에 SRV·샘플러 + 셰이더에 `tex.Sample` + 머티리얼→파일 해석(FBX 는 stale `.psd` 경로 → 이름 매칭으로 로컬 `.tga`: body→body_01.tga 등). UV 는 이미 `ModelVertex.uv` 에 있음(48k/48k 정점) | TGA 자체 구현 vs stb_image vendor, 경로 매칭 규칙, 노멀맵·스페큘러까지 갈지 | `src/render/r3d/ModelMeshPass3D.*`, 새 `src/import/` 또는 `src/vendor/` 로더, `src/import/Model.h` |
