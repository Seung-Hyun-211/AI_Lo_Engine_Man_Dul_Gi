...
| 4 | "실제 게임 시작하자 / 씬·엔티티 만들어" | **씬/스테이트 됨**: `GameState{Title,InGame}` + Settings 모달 오버레이, `docs/scene-flow-design.md`. 남은 일: `src/game/`에 엔티티 표현(초기엔 AoS `vector<Entity>`), `Simulation`의 데모 페이로드(파티클+회전 큐브)를 실제 콘텐츠로 교체. `SnapshotBuilder`가 엔티티 → `MeshDraw`/`Quad`. 장르·시놉시스는 `docs/synopsis.md`(초안, 미확정) | 장르·핵심 루프(2D인지 3D인지) — `docs/synopsis.md` 확정 필요. 엔티티 모델(컴포넌트? 단순 struct?). 데모 페이로드 유지 여부 | `src/game/Simulation.*`, `src/game/SnapshotBuilder.*`, 새 `src/game/*` |
...
- 상세 계약: `docs/engine-overview.md`, `multithreaded_game_engine_architecture.md`, `ui-architecture.md`, `scene-flow-design.md`, `game-settings.md`, `synopsis.md`, `time-design.md`, `collider-design.md`, `model-animation-research.md`, `animation-design.md`, `shader-pipeline.md`, `toon-rendering.md`, `lighting.md`, `msaa.md`, `shadows.md`.
