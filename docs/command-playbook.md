...
| 4b | "3D 카메라 조작 붙여 (WASD·마우스 등)" | **됨 (3인칭 오빗 팔로우 + 마우스 잠금)**: `Simulation` 이 `m_cameraYaw/Pitch` + `m_actors[0]`(플레이어) 보유, `UpdateCameraLook`(프레임당 1회) + `StepActors`/`StepOneActor`(고정 스텝). `Application::BuildPlayerIntent` 가 WASD/Shift/Space/MouseDelta → `PlayerIntent`. `Win32Window::SetPointerLocked` 가 raw input + 커서 잠금. `SnapshotBuilder::BuildCamera` 가 그 상태로 view. 상세·튜닝은 `docs/demo-scene.md` | FPS / 오빗 / 고정 (현재 오빗). 감도·역전은 `Simulation` 상수 (설정 UI 연결은 안 함) | `src/game/Simulation.*`, `src/game/Application.*`, `src/game/SnapshotBuilder.cpp`, `src/platform/Win32Window.*`, `src/input/InputState.h` |
...
- 상세 계약: `docs/engine-overview.md`, `multithreaded_game_engine_architecture.md`, `ui-architecture.md`, `scene-flow-design.md`, `game-settings.md`, `synopsis.md`, `entity-lifecycle-design.md`, `loading-and-streaming.md`, `time-design.md`, `collider-design.md`, `model-animation-research.md`, `animation-design.md`, `shader-pipeline.md`, `toon-rendering.md`, `lighting.md`, `msaa.md`, `shadows.md`.
