...
- `Step()` 밖에서 애니메이션 시간 전진 (고정 timestep 규칙) — `CharacterAnimationState::Tick` 도 `Simulation::StepDemo3D` 안에서만.
