...
| `char/<id>` | 플레이어블 1명 전 클립 + 초상화 (`char/magic_knight` …) | 그 캐릭터 선택 시 | 1024 (넘치면 2048) / 0 / 2~4 |
...
| 분류 | 접두어 | 예 (파일 stem) | 그룹 |
|---|---|---|---|
| 플레이어블 | `char` | `char_magic_knight_walk_00`, `char_magic_knight_portrait` | `char/<id>` |
...
캐릭터 id: `magic_knight`(마도기사) `skull_magician`(스컬매지션) `succubus`(서큐버스) `assassin_lizard`(어쎄신리자드).
...
- 시점: 기본(접미어 없음) = **오른쪽을 보는 옆모습**. 왼쪽은 **런타임에서 좌우 반전**하므로 **왼쪽 프레임을 따로 그리지 않는다**. 위/아래를 보는 그림이 꼭 필요할 때만 `_up` / `_down` 을 상태 뒤에 붙인다(`char_magic_knight_walk_up_00`).
...
| 캔버스 크기 | **플레이어 64×128 [확정], 히트박스 48×48 은 캔버스 맨 아래 가운데**(`player.csv` `sprite_*`/`hitbox_*`), [살] 일반 몹 32×32(반경 10 기준 — 탱커 48×48), 보스 128~192, 이펙트 32~128, 타일 64×64, 무기/장신구 아이콘 48×48, 초상화 ≈176×168(HUD 좌상 1 실측) |
| 원점(pivot) | 캐릭터·몹·보스는 **캔버스 하단 중앙(발밑)** — 그림은 **히트박스(콜라이더) 위에 서 있다**: 캔버스 아래 변 = 히트박스 아래 변, 가로 가운데 정렬(플레이어 48×48 박스, 몹 반경 `mob_radius` 원). 이펙트·투사체·아이콘은 **중앙**. 패커는 pivot 을 안 쓰므로(기본 0.5,0.5) **`anim_clips.csv` 의 `pivot_x/pivot_y` 로 지정**(§6.1) |
| 방향 | 원본은 top-down 행 순서 그대로(뒤집지 말 것) |
| 큰 배경 | 페이지(최대 4096)보다 큰 그림은 아틀라스에 못 넣는다(패커 에러). 큰 배경은 타일로 쪼개거나 단독 텍스처 경로가 필요([미정], 지금 없음) |

### 4.1 템플릿 — `assets/templates/` ✅

**새 그림은 템플릿 위에 그린다.** 템플릿은 실제 게임 크기(월드 1단위 = 1px, 1920×1080 화면 기준, 레벨 1·`attack_size` 1)로 만든
가이드 그림이다 — 레이어로 깔고 그린 뒤 **가이드 레이어는 지우고** §3 이름으로 `assets/src/<그룹>/` 에 저장한다.
`assets/templates/` 는 패킹 대상이 아니다(아틀라스에 안 들어감).

| 표시 | 뜻 |
|---|---|
| 자홍 1px 테두리 | 캔버스 = 그림의 크기. 이 크기로 저장 |
| 하늘색 영역 | **판정(히트박스·공격 범위)**. 그림이 맞는 영역을 보여 주는 기준 |
| 노란 십자 | 게임 위치(히트박스 중심, 공격의 원점) |
| 빨간 화살표 | 기본 방향 = 오른쪽. 왼쪽은 런타임 좌우 반전(§3.3) |

| 파일 | 크기 | 내용 |
|---|---|---|
| `char/char_template.png` | 64×128 | 플레이어. 히트박스 48×48 이 캔버스 맨 아래 가운데 |
...
| `fx/fx_hit_template.png` | 32×32 | 명중 이펙트(중심 원점) |

- **크기 규칙**: 공격 그림은 런타임에 **실제 판정 크기에 맞춰 늘어난다**(레벨·`attack_size`로 범위가 커지면 그림도 커짐 — "보이는 크기 = 맞는 크기",
...
1. **원본 배치** — `assets/src/char/magic_knight/` 아래에 §3 규칙 이름으로 저장:
   `char_magic_knight_walk_00.png … char_magic_knight_walk_05.png`, `char_magic_knight_portrait.png`
   (하위 폴더 `walk/` 등으로 정리해도 되지만 **stem 은 그룹 전체에서 유일**해야 한다).
2. **그룹 등록** — `assets/atlas/atlas.groups` 에 없으면 한 줄 추가:
   ```
   group char/magic_knight page 1024 mips 0 gutter 2
   ```
...
4. **굽기** — 저장소 루트에서:
   ```bash
   build\tools\atlas_pack.exe --group char/magic_knight
   ```
   (`--all` 전체, `--force` 캐시 무시 재빌드.) 성공하면 `assets/atlas/char_magic_knight.0.dds` + `char_magic_knight.atlas` 가 생긴다.
...
5. **확인** — `.atlas` 를 열어 이름·개수를 본다(한 줄 = 한 스프라이트):
   ...
   char_magic_knight_walk_00 0 0.001953 0.001953 0.033203 0.033203 32 32
   ```
...
- **프레임은 이름으로 찾는다**: 클립 `char_magic_knight_walk` 는 `char_magic_knight_walk_00`, `_01`, … 를 **연속으로 만나는 데까지**가 프레임이다(끊기면 거기가 끝). 프레임 목록을 어디에도 적지 않는다.
- **재생 속성은 표로**: `assets/data/circular/anim_clips.csv` (예정 — 밸런스 CSV 와 같은 로더 [circular-balance.md](circular-balance.md)):

| 열 | 뜻 | 기본 |
|---|---|---|
| `clip` | 프레임 접두어(=클립 id), 예 `char_magic_knight_walk` | 필수 |
| `fps` | 초당 프레임 | 10 |
...
char_magic_knight_idle,6,loop,,0.5,1.0,
char_magic_knight_walk,10,loop,,0.5,1.0,
char_magic_knight_run,14,loop,,0.5,1.0,
char_magic_knight_dash,,once,0.18,0.5,1.0,
...
2. `assets/templates/` 의 템플릿(§4.1) 위에 그리고, §3 규칙으로 이름을 짓고 §4 사양으로 저장 → `assets/src/<그룹>/`.
