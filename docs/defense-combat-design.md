...
**패배는 결과 화면 없이 즉시 로비 메뉴로** — §0.3이 없어서 "패배" 표시 없이 바로 `EnterLobby()`만
...
Combat(60s)  ──타이머 만료──▶  전멸 처리(§0.2)  ──▶  WaveResults(§0.3) │
  ...
  좀비 무한 리스폰(horde §4.5)                              │           │
  목표HP 0 되면──▶ EnterLobby() (구현됨, 결과화면 없이 즉시)  │ "다음으로" │
                                                              ▼           │
                                                     Prep(60s)(§0.4) ─────┘
                                                       waveActive=false
                                                       배치/구매만, 좀비 없음
...
- **승패**: 목표 HP 0 = 즉시 패배(웨이브 진행 중 언제든) → `EnterLobby()`. 그 외엔 무한 반복
  (승리 조건 없음 — 엔드리스 웨이브 디펜스, `synopsis.md` 장르 그대로). 최고 웨이브 도달 수가
  스코어(아직 기록 안 함 — §0.3에서 할 일).

  **구현노트**: 결과 화면(§0.3, "패배" 표시 + "로비로" 버튼)이 아직 없어서 최소 연결만 함 —
  `Application::Run()`의 프레임 루프가 스텝 직후 `m_simulation.IsMatchLost()`
  (`m_objectiveHealth <= 0.0f`)를 보고 참이면 그 프레임에 바로 `EnterLobby()`. 확인 화면·
  "다음으로/로비로" 버튼 분기 없이 한 프레임 만에 로비 메뉴로 끊긴다. 재시작은 `EnterInGame()`이
...
정산 화면이 생기면 같이 정리할 대상. "다음으로/로비로" 버튼이나 웨이브 종료 시점의 별도 화면 전환은 없음 — 그냥 매
프레임 최신 값이 계속 갈아치워진다. FPS 카운터와 마찬가지로 로비 화면 위에도 그려짐(게이팅
...
- 패배였으면 버튼이 "다음으로" 대신 "로비로" → `EnterLobby()`.
