...
**패배는 결과 화면 없이 즉시 씬 선택 메뉴로** — §0.3이 없어서 "패배" 표시 없이 바로 `EnterSceneSelect()`만
...
Combat(60s)  ──타이머 만료──▶  전멸 처리(§0.2)  ──▶  WaveResults(§0.3) │
  ...
  좀비 무한 리스폰(horde §4.5)                              │           │
  목표HP 0 되면──▶ EnterSceneSelect() (구현됨, 결과화면 없이 즉시)  │ "다음으로" │
                                                              ▼           │
                                                     Prep(60s)(§0.4) ─────┘
                                                       waveActive=false
                                                       배치/구매만, 좀비 없음
...
- **승패**: 목표 HP 0 = 즉시 패배(웨이브 진행 중 언제든) → `EnterSceneSelect()`. 그 외엔 무한 반복
  (승리 조건 없음 — 엔드리스 웨이브 디펜스, `synopsis.md` 장르 그대로). 최고 웨이브 도달 수가
  스코어(아직 기록 안 함 — §0.3에서 할 일).

  **구현노트**: 결과 화면(§0.3, "패배" 표시 + "씬 선택으로" 버튼)이 아직 없어서 최소 연결만 함 —
  `Application::Run()`의 프레임 루프가 스텝 직후 `m_simulation.IsMatchLost()`
  (`m_objectiveHealth <= 0.0f`)를 보고 참이면 그 프레임에 바로 `EnterSceneSelect()`. 확인 화면·
  "다음으로/씬 선택으로" 버튼 분기 없이 한 프레임 만에 씬 선택 메뉴로 끊긴다. 재시작은 `EnterInGame()`이
...
정산 화면이 생기면 같이 정리할 대상. "다음으로/씬 선택으로" 버튼이나 웨이브 종료 시점의 별도 화면 전환은 없음 — 그냥 매
프레임 최신 값이 계속 갈아치워진다. FPS 카운터와 마찬가지로 씬 선택 화면 위에도 그려짐(게이팅
안 함) — 개발용 상시 리드아웃이라는 성격상 지금은 문제로 안 봄. 진짜 정산 화면(웨이브 종료
때 잠깐 멈추고 "다음으로" 확인)은 아래 스케치 그대로 여전히 미구현.

`scene-flow-design.md`가 이미 예비해 둔 `GameState` 값 — Menu/InGame과 똑같이 `SetScreen`
(전체화면 교체), `Simulation::Step` 은 Menu 처럼 건너뜀(오버레이 아니라 상태 전환이므로
...
- 패배였으면 버튼이 "다음으로" 대신 "씬 선택으로" → `EnterSceneSelect()`.
...
- **정비 페이즈 UI(상점 패널)**: §0.1 자원 획득/소비 로직은 이미 구현됐지만(배치 함수 자체가
  자금 게이트) 그걸 보여주는 화면은 없음 — HUD 갱신 방식(`SnapshotBuilder` 텍스트 HUD — `InGameHud` 위젯은 삭제됨) 결정
  후 착수.
