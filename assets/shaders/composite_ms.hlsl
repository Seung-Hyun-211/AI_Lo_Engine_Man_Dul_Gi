...
// docs/post-process-gbuffer-research.md §4.3/§6/§12.7/§12.11 step 3. Two files
// instead of one because HLSL has no Texture2D/Texture2DMS polymorphism and this
// engine has no shader-permutation system yet (docs/shader-pipeline.md "다음").
...
float4 PSMain(VSOut input) : SV_TARGET
{
    ...
    return float4(sum / max(sampleCount, 1u), 1.0f);
}
