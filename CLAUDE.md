...
- **`UIContext`의 화면 교체가 즉시 파괴다** — `SetScreen`/`SetOverlay`/`ClearOverlay`에 지연 큐가 없어, 위젯 콜백에서 화면을 바꾸면 실행 중인 `onClick`(`std::function`)이 그 안에서 파괴된다(`CLOSE` 버튼이 이미 그 경로 — 표준상 UB이나 `Button::PointerUp`이 콜백 뒤 멤버를 안 만지고 지역변수를 반환해 현재는 동작). `ui-architecture.md` "화면 교체 시점"에 불변식과 해결책(지연 슬롯) 정리. 언어 전환 재빌드(`localization-design.md` §5) 전에 고치는 게 좋다.
